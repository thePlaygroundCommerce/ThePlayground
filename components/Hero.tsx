...
const Hero = ({
    ...
    return (
        <Carousel>
            {() => items.map(({ imageProps, title, last_publication_date }, i) => (
                <div key={i} className="w-full h-full relative">
                    ...
                    <div className="absolute z-20 w-full h-full flex justify-center text-center items-center">
                        <div>
                            ...
                            <p className="text-lg">{new Date(last_publication_date).toDateString()}</p>
                            <Link href=""><Button variant='primary'>READ MORE</Button></Link>
                        </div>
                        {/* {use_logo && (
